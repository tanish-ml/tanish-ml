<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=19,23&height=250&section=header&text=TANISH%20MITTAL&fontSize=50&fontColor=ffffff&animation=fadeIn&fontAlignY=35&desc=DATA%20SCIENTIST%20%7C%20AI%20ARCHITECT&descAlignY=55&descAlign=50" />
</div>

<div align="center">
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.demolab.com?font=Orbitron&weight=600&size=20&pause=1000&color=00FF99&center=true&vCenter=true&width=600&lines=Building+Scalable+AI+Systems;Mastering+PyTorch+%26+Deep+Learning;Turning+Data+into+Actionable+Intelligence;Open+to+Collaboration" alt="Typing SVG" />
  </a>
</div>
 
<br><br>

<table width="100%" align="center" style="border: none;">
  <tr>
    <td width="40%" align="center">
      <img src="https://media.giphy.com/media/LmNwrBhejkK9EFP504/giphy.gif" width="300" style="border-radius: 10px; box-shadow: 0 0 15px #00FF99;" alt="Data Matrix" />
    </td>
    <td width="60%">
      <h2>🌌 <b>System Directives</b></h2>
      <p><b>Designation:</b> Machine Learning Engineer & Data Analyst</p>
      <p><b>Mission:</b> Transforming complex data into scalable AI solutions.</p>
      <p><b>Current Directive:</b> Exploring advanced Deep Learning methodologies and optimizing Neural Networks.</p>
      <p><b>Status:</b> Always open to contributing to open-source AI/DS projects.</p>
    </td>
  </tr>
</table>

<br><br>

<div align="center">
  <h2>🧠 <b>Data Science Workflow</b></h2>
</div>

```mermaid
graph TD;
    classDef core fill:#1F2937,stroke:#00FF99,stroke-width:2px,color:#fff;
    classDef proc fill:#000000,stroke:#60A5FA,stroke-width:2px,color:#fff;
    
    A[(Raw Data)]:::core --> B(Feature Engineering):::proc;
    B --> C{Model Selection}:::core;
    C -->|PyTorch| D[Deep Learning]:::proc;
    C -->|Scikit-Learn| E[Classical ML]:::proc;
    D --> F((Deployment)):::core;
    E --> F;
```

<br><br>

<div align="center">
  <h2>⚙️ <b>Neural Architecture (Tech Stack)</b></h2>
  <br>
  <img src="https://skillicons.dev/icons?i=python,pytorch,tensorflow,scikitlearn,pandas,numpy&theme=dark&perline=6" /><br><br>
  <img src="https://skillicons.dev/icons?i=docker,linux,aws,gcp,git,bash&theme=dark&perline=6" />
</div>

<br><br>

<div align="center">
  <h2>💬 <b>Daily Developer Quote</b></h2>
  <br>
  <img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight" alt="Daily Quote" />
</div>

<br><br>

<div align="center">
  <h2>🐍 <b>Contribution Heatmap</b></h2>
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/tanish-ml/tanish-ml/output/github-contribution-grid-snake-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/tanish-ml/tanish-ml/output/github-contribution-grid-snake.svg">
    <img alt="github contribution grid snake animation" src="https://raw.githubusercontent.com/tanish-ml/tanish-ml/output/github-contribution-grid-snake.svg">
  </picture>
</div>

<br><br>

<div align="center">
  <h2>📡 <b>Establish Uplink</b></h2>
  <br>
  <a href="mailto:tanishmittal2212@gmail.com">
    <img src="https://img.shields.io/badge/Email-000000?style=for-the-badge&logo=gmail&logoColor=00FF99" />
  </a>
  <a href="https://github.com/tanish-ml">
    <img src="https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github&logoColor=00FF99" />
  </a>
  <a href="https://tanish-ml.github.io/tanish-ml-portfolio/">
    <img src="https://img.shields.io/badge/GitHub%20Pages-121013?style=for-the-badge&logo=github&logoColor=00FF99" />
  </a>
</div>

<br>

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=19,23&height=100&section=footer" />
</div>
